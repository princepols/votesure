...
// Confirm still registered (extra safety)
...
if (!$chk->fetch()) {
    ...
    <html lang="en"><head>
      ...
      <style>
        body{font-family:Poppins, sans-serif;background:#f8f9fb;display:flex;justify-content:center;align-items:center;height:100vh;margin:0;}
        .box{background:#fff;padding:40px;border-radius:12px;box-shadow:0 4px 10px rgba(0,0,0,0.1);text-align:center;border-top:8px solid #800000;}
        h2{color:#800000;margin-bottom:10px;} p{color:#555;margin-bottom:20px;}
        a{background:#800000;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:600;}
        a:hover{background:#660000;}
      </style>
    </head><body>
      <div class="box">
        <h2>Access Denied</h2>
        <p>This Student ID is no longer registered. Please contact the administrator.</p>
...
if (!$election) {
    ...
    <head>
      ...
      <style>
        body{background-color:#f2f2f2;font-family:Poppins,sans-serif;display:flex;justify-content:center;align-items:center;height:100vh;margin:0;}
        .container{background:#fff;border-radius:12px;box-shadow:0 4px 10px rgba(0,0,0,0.1);padding:40px;text-align:center;width:400px;border-top:8px solid #800000;}
        h1{color:#800000;font-size:1.8rem;margin-bottom:15px;} p{color:#555;margin-bottom:25px;}
        a.button{display:inline-block;background-color:#800000;color:#fff;padding:12px 25px;border-radius:6px;text-decoration:none;font-weight:600;transition:background 0.3s ease;}
        a.button:hover{background-color:#660000;}
      </style>
    </head>
    <body>
      <div class="container">
        <h1>No Active Election</h1>
        <p>There is currently no ongoing election. Access Denied.</p>
...
$stmt = $pdo->prepare("
  SELECT p.id AS pid, p.name AS pname,
         c.id AS cid, c.name AS cname, c.photo AS cphoto, c.position AS cpos
  FROM partylists p
...
foreach ($rows as $r) {
    if (!isset($partylists[$r['pid']])) {
        $partylists[$r['pid']] = ['name' => $r['pname'], 'candidates' => []];
    }
    if ($r['cid']) {
        $partylists[$r['pid']]['candidates'][] = [
            'id' => $r['cid'],
            'name' => $r['cname'],
            'photo' => $r['cphoto'],
            'position' => $r['cpos']
  ...
  <div class="col-md-10 offset-md-1">
    <div class="card card-modern p-4">
      <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div class="text-end small-muted">Voter ID: <?php echo h($student_id); ?></div>
      </div>
                      ...
                      <div class="p-2">
                        ...
                        <div class="mt-2 d-flex justify-content-between align-items-center">
                          <div class="form-check">
                            <input class="form-check-input radio-choice" type="radio" name="choice[<?php echo intval($pid); ?>]" value="<?php echo intval($cand['id']); ?>">
                          </div>
...
document.querySelectorAll('.select-btn').forEach(btn=>{
  btn.addEventListener('click', ()=>{
    ...
    card.querySelector('.radio-choice').checked = true;
...
document.getElementById('reviewBtn').addEventListener('click', ()=>{
  ...
  if (selected.length < expected) { alert('Please select one candidate for each section.'); return; }
  let html = '<ul class="list-group">';
  selected.forEach(s=>{
    const card = s.closest('.candidate-card');
    const lbl = card.querySelector('.fw-semibold').innerText;
    const pos = card.querySelector('.small-muted').innerText;
...
document.getElementById('submitVote').addEventListener('click', ()=> document.getElementById('voteForm').submit());
</script>
